...
Не все СУБД умеют работать распределено. 
...
-- это как раз Hive. Но не будем называть Hive СУБД, об этом ниже.
...
Мы будем в основном работать с External таблицами, так как касаемся задач "посчитать статистику". 

Утверждение: Hive не является СУБД. Суть Hive в том, что SQL запрос превращается в последовательность MapReduce задач. 
MapReduce задачи работают с данными в HDFS. Чтобы Hive посчитал статистики, нужно задать Hive правила, по которому он должен парсить данные в HDFS.
В первой задаче из ДЗ нужно будет создать таблицы, чтобы с ними работать в следующих задачах. 
Процесс создания таблиц в Hive в основном состоит из написания регулярных выражений для парсинга данных в HDFS.

Получается, что мы все работаем с одними и теми же данными в HDFS, но каждый создает свой набор таблиц по этим данным, и может реализовать произвольную логику работы с этими данными.
Если кто-то решит удалить свою таблицу, то исходные данны не изменятся. Поэтому используем таблицы External.
