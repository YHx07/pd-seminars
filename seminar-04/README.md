...
В качестве эксперимента попробуем ILP - Instruction-level parallelism или параллелизм на уровне инструкций. Что это значит - в целом, за счёт знания архитектуры можно выйграть в игру оптимизацию.
...
Теперь передаём в add numBlocks/ILP блоков. Теперь ещё требуется ядро - одно правильное и одно не правильное. Идея для порчи скорости - требуется доступ к памяти, которая не совпадает с той, которая на текущий момент пришла к потоку. Испортим все так: просто постучимся в следующую ячейку вместо оптимальной.
