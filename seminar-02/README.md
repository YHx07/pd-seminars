...
```
8. ```MPI_Recv(...)``` - Используется для получения сообщений из выбранного коммуникатора от выбранного процесса. Параметры аналогичны `MPI_Send` за исключением `MPI_Status` - он используется для хранения данных об сообщении в случае, если мы получили его без указания получателя или подобного.
...
MPI предоставляет систему для передачи сообщений между процессами и для параллельного запуска процессов на разных машинах. Однако, для общения между процессами, запущенными на одной машине (модель общей памяти), такой формат работы является перегруженным, так как нам приходится отправлять сообщения через внешнюю систему на ту же машину. В то же время мы могли бы просто распараллеливать один процесс на несколько потоков, получив выигрышь в производительности.

OpenMP имеет директивы компилятора, которые явно говорят, что следующий блок кода требуется расспаралелить на множество потоков и потом как то с агрегировать. При этом, если компилятор не поддерживает OpenMP, то такой код с большой вероятностью будет просто выполнен в 1 потоке. 

В программах с OpenMP используется хедер: ```#include<omp.h>```.
...
1. ```gcc -fopenmp main.c``` / ```g++ -fopenmp main.cpp```
2. ```./a.out```
...
1. ```cmake .```
2. ```make```
3. ```./a.out``` -- или другое название, указанное в CMakeLists.txt
