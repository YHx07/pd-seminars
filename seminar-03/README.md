...
- Thread -- поток -- уникальная единица вычисления в видеокарте.  Каждый поток выполняет одну и ту же программу, но на разных данных. Потоки организованы в блоки, и каждый поток внутри блока имеет уникальный идентификатор.
- Warp -- группа из 32 потоков (в текущих архитектурах), которые физически отрабатывают за один такт времени и исполняют одну программу. На практике это означает, что одну команду посылаем всем потокам в Warp-е, GPU сама объединяет потоки (thread-ы) в warp-ы.
...
- Grid -- структура, состоящая из одного или нескольких блоков, количество блоков в grid-е задается пользователем. Grid организует выполнение всех блоков, которые совместно решают одну задачу. Гриды могут быть одномерными, двумерными, трехмерными. Например, для задач трехмерной компьютерной графики будут полезны трехмерные гриды.
 
- Streaming Multiprocessor (SM) -- аналог физических ядер, аппаратная единица внутри GPU, которая отвечает за выполнение потоков. Один блок обрабатывается одним SM. Каждый SM содержит несколько ядер CUDA, которые исполняют потоки, а также имеет свои собственные регистры, общую память и другие аппаратные ресурсы.
